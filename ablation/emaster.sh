        ...
        for weight_clip in True False; do
          for t_mapping in RandomBETMapping None; do
            for e_mapping in None RowHammerUpSprayAttackMapping RowHammerSprayAttackMapping; do
              ...
              sbatch ./parameterized.sh --train=False --epoch=3000 --dropout_p=$p --sigma=$sigma --first_dropout=$first_dropout --last_dropout=$last_dropout --weight_clip=$weight_clip --t_mapping=$t_mapping --e_mapping=$e_mapping
            done
          done
        done
      done
